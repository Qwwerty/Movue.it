...
export default {
	[Mutations.SET_CURRENT_CHALLENGE_INDEX](state, index) {
		state.curretnChallengeIndex = index;
	},
	[Mutations.SET_IS_LEVEL_UP_MODEL_OPEN](state, flag) {
	...
	[Mutations.COMPLETE_CHALLENGE](state, xpAmount) {
		...
		if (shouldLevelUp) {
			...
			const expecienceToNextLevel = Math.pow((state.level + 1) * 4, 2);

			state.xp = {
				current: remainingXp,
				start: 0,
				end: expecienceToNextLevel,
			};
